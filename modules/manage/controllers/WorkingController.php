...
use app\modules\manage\models\WorkingSearch;
...
use app\modules\manage\models\WorkingSearchIso;
...
class WorkingController extends Controller
{
     ...
     * @return mixed
     */
    public function actionIndexIso($idiso, $idEx=NULL)
    {
        $searchModel = new WorkingSearchIso();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams,$idiso, $idEx);
        $iso = Iso::findOne($idiso);
        $exam = Examination::findOne($idEx);
        if($iso != null){
            return $this->render('indexIso', [
                'searchModel' => $searchModel,
                'dataProvider' => $dataProvider,
                'idiso'=>$idiso,
                'iso'=>$iso,
